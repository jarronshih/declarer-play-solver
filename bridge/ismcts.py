...
class Node:
    ...
    def __repr__(self):
        return "[M:%s W/V/A: %4.4f/%4i/%4i]" % (self.move, self.wins / self.visits, self.visits, self.avails)

    def TreeToString(self, indent):
        """ Represent the tree as a string, for debugging purposes.
        """
        s = self.IndentString(indent) + str(self)
        for c in sorted(self.childNodes, key=lambda n: n.wins / n.visits, reverse=True):
